...
const worker = async () => {
  ...
  let parseDepositStocks = $('#example tbody tr').map((index, stock) => {
      return {
        symbol: $(stock).children("td").eq(0).text(),
        company: $(stock).children("td").eq(1).text() || $(stock).children("td").eq(0).text(),
        price: parseFloat($(stock).children("td").eq(3).text()),
      }
    }).get();
  ...
  for(let stock of parseDepositStocks) {
    ...
    const dividendList = _.reduce($('#divDetail tbody tr'), (accu, item, index) => {
